...
export class GitSync {
  ...
  private async pruneTrackedDerivedFiles(): Promise<void> {
    try {
      ...
      console.error(
        `[private-journal] untracked ${tracked.length} derived file(s); local copies kept`,
      );
    } catch (err) {
  ...
  async commitAndPush(
    ...
    return this.trackingChangedMarkdown(() =>
      this.withLock(() => this.commitAndPushUnlocked(message, supportedVersion, options)),
    );
  }

  private async commitAndPushUnlocked(
    message: string,
